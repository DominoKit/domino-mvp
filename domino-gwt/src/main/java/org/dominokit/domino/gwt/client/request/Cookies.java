...
public class Cookies {
  ...
  private static void setCookieImpl(
      String name, String value, long expires, String domain, String path, boolean secure) {
    String c = name + "=" + value;
    if (expires > 0) c += ";expires=" + (new Date(expires)).toGMTString();
    if (nonNull(domain) && !domain.isEmpty()) c += ";domain=" + domain;
    if (nonNull(path) && !path.isEmpty()) c += ";path=" + path;
    if (secure) c += ";secure";
