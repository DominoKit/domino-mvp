...
public class TestConfiguration implements ServerConfiguration<Object, Object[]> {
  ...
  public Integer getInteger(String key, Integer def) {
    if (config.containsKey(key)) return getInteger(key);
    return def;
  }

  @Override
  public Long getLong(String key, Long def) {
    if (config.containsKey(key)) return getLong(key);
    return def;
  }

  @Override
  public Double getDouble(String key, Double def) {
    if (config.containsKey(key)) return getDouble(key);
    return def;
  }

  @Override
  public Float getFloat(String key, Float def) {
    if (config.containsKey(key)) return getFloat(key);
    return def;
  }
  ...
  public Object getJsonObject(String key, Object def) {
    if (config.containsKey(key)) return getJsonObject(key);
    return def;
  }

  @Override
  public Object[] getJsonArray(String key, Object[] def) {
    if (config.containsKey(key)) return getJsonArray(key);
    return def;
  }

  @Override
  public byte[] getBinary(String key, byte[] def) {
    if (config.containsKey(key)) return getBinary(key);
    return def;
  }

  @Override
  public Instant getInstant(String key, Instant def) {
    if (config.containsKey(key)) return getInstant(key);
    return def;
  }

  @Override
  public Object getValue(String key, Object def) {
    if (config.containsKey(key)) return getValue(key);
    return def;
  }
