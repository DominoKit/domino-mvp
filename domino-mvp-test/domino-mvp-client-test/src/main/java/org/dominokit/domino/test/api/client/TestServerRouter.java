...
public class TestServerRouter implements RequestRouter<ServerRequest> {
  ...
  private void completeFailRequest(ServerRequest request, FailedReply reply) {
    String requestKey = getRequestKey(request);
    Promise<ResponseReply> p = requestCompleteHandlers.remove(requestKey);
    if (p != null) {
      p.fail(new TestFailedRequestException(reply)); // or p.tryFail(...)
    }
  }

  // success path (for reference):
  private void completeOkRequest(ServerRequest request, ResponseReply reply) {
    String requestKey = getRequestKey(request);
    Promise<ResponseReply> p = requestCompleteHandlers.remove(requestKey);
    if (p != null) {
      p.complete(reply); // or p.tryComplete(reply)
    }
  }
