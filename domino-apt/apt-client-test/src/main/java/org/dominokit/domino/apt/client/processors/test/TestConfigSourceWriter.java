...
public class TestConfigSourceWriter extends AbstractSourceBuilder {
    ...
    public List<TypeSpec.Builder> asTypeBuilder() {
        ...
        TypeSpec.Builder testConfigType = DominoTypeBuilder.classBuilder(testConfigClassName, TestConfigProcessor.class)
    ...
    private void generateModulesMethod(TypeSpec.Builder testConfigType) {
        List<TypeMirror> modules = processorUtil.getClassArrayValueFromAnnotation(testClassElement, TestConfig.class, "modules");

        MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder("getModules")
                .addAnnotation(Override.class)
                .addModifiers(Modifier.PUBLIC)
                .returns(ParameterizedTypeName.get(ClassName.get(List.class), TypeName.get(ModuleConfiguration.class)))
                .addStatement("$T<$T> modules = new $T<>()", TypeName.get(List.class), TypeName.get(ModuleConfiguration.class), TypeName.get(ArrayList.class));

        modules.forEach(typeMirror -> methodBuilder.addStatement("modules.add(new $T())", TypeName.get(typeMirror)));
    ...
    private void addModuleByBestGuess(MethodSpec.Builder methodBuilder, String moduleName) {
        methodBuilder.addStatement("modules.add(new $T())", ClassName.bestGuess(elements.getPackageOf(testClassElement).getQualifiedName().toString() + "." + (moduleName + "ModuleConfiguration")));
    }


    private void generateOnBeforeClientStartMethod(TypeSpec.Builder testConfigType) {

        MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder("onBeforeClientStart")
                .addAnnotation(Override.class)
                .addModifiers(Modifier.PUBLIC)
                ...
                .addStatement("$T test = ($T) dominoTestCase", testClassElement.asType(), testClassElement.asType());

        testClassElement.getEnclosedElements()
                .stream()
                .filter(element -> nonNull(element.getAnnotation(OnBeforeClientStart.class)))
                .filter(element -> ElementKind.METHOD.equals(element.getKind()))
                .filter(element -> element.getModifiers().contains(Modifier.PUBLIC) || element.getModifiers().contains(Modifier.DEFAULT))
                .forEach(element -> {
                    String args = getElementArguments(element);

                    methodBuilder.addStatement("test." + element.getSimpleName().toString() + "(" + args + ")");
                });
    ...
    private void generateOnClientStartedMethod(TypeSpec.Builder testConfigType) {
        ...
        MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder("onClientStarted")
                .addAnnotation(Override.class)
                .addModifiers(Modifier.PUBLIC)
                .returns(TypeName.VOID)
                .addParameter(TypeName.get(DominoTestCase.class), "dominoTestCase")
    ...
    private String getParamName(VariableElement param) {
        if (isClientContext(param))
            return "clientContext";
        else if (isTestContext(param))
            return "testContext";
        return param.getSimpleName().toString();
    }

    private boolean isClientContext(VariableElement param) {
        return types.isSameType(param.asType(), elements.getTypeElement(ClientContext.class.getCanonicalName()).asType());
    }

    private boolean isTestContext(VariableElement param) {
        return types.isSameType(param.asType(), elements.getTypeElement(TestContext.class.getCanonicalName()).asType());
    }

    private void generateBindSpiesMethod(TypeSpec.Builder testConfigType) {

        MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder("bindSpies")
                .addAnnotation(Override.class)
                .addModifiers(Modifier.PUBLIC)
                .returns(TypeName.VOID)
                .addParameter(TypeName.get(DominoTestCase.class), "dominoTestCase")
    ...
    private CodeBlock generateSpyBinding(Element element) {
        ...
        if(targetPresenter.isPresent()){
            ...
            boolean isViewable = processorUtil.isAssignableFrom(presenterTypeElement, ViewBaseClientPresenter.class);
    ...
    private void generateBindFakeViewsMethod(TypeSpec.Builder testConfigType) {
        ...
        MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder("bindFakeViews")
                .addAnnotation(Override.class)
                .addModifiers(Modifier.PUBLIC)
                .returns(TypeName.VOID)
                .addParameter(TypeName.get(DominoTestCase.class), "dominoTestCase")
    ...
    private CodeBlock generateFakeViewsBinding(Element element) {
        Optional<TypeMirror> targetPresenter = processorUtil.getClassValueFromAnnotation(element, FakeView.class, "value");
        if(targetPresenter.isPresent()){
            ...
            ClassName configType = ClassName.bestGuess(elements.getPackageOf(presenterTypeElement).toString() + "." + configName);
            builder.addStatement("$T $L = new $T()", configType, processorUtil.smallFirstLetter(configName), configType);
            builder.beginControlFlow("$L.setViewSupplier(() ->", processorUtil.smallFirstLetter(configName));
    ...
    private String getBindingConfigName(Element presenterTypeElement) {
        String configName;
        if (nonNull(presenterTypeElement
                .getAnnotation(PresenterProxy.class))) {
